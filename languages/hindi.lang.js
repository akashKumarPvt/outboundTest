...
export async function getDtmf(dtmf, person1) {
  if (dtmf === "1" || dtmf === "") {
    await setDtmfUser("dtmf2", person1);
    await setUserChoice("hindi", person1);
    ...
    return getDtmf("1", person1);
  }
}

export async function getDtmf2(dtmf, person1) {
  if (dtmf == "1") {
    ...
    await setDtmfUser("dtmf2", person1);
    return getDtmf2("1", person1);
  }
}

export async function getDtmf3(dtmf, person1) {
  if (dtmf === "1") {
    ...
    await setDtmfUser("dtmf3", person1);
    return getDtmf2("1" || "0", person1);
  }
}

export async function getDtmf4Sec1(dtmf, person1) {
  if (dtmf === "1" || dtmf === "2" || dtmf === "3") {
    ...
    await setDtmfUser("dtmf41", person1);
    return getDtmf3("1", person1);
  }
}

export async function getDtmf4Sec2(dtmf, person1) {
  if (dtmf === "1" || dtmf === "2" || dtmf === "3" || dtmf === "4") {
    ...
    await setDtmfUser("dtmf42", person1);
    return getDtmf3("2", person1);
  }
}

export async function getDtmf5(dtmf, person1) {
  if (dtmf === "1") {
...
export async function getDtmf5Sec1(dtmf, person1) {
  if (dtmf >= "1" && dtmf <= "7") {
    ...
    await setDtmfUser("dtmf51", person1);
    return getDtmf5("1", person1);
  }
}

export async function getDtmf5Sec2(dtmf, person1) {
  if (dtmf === "1") {
    await setDtmfUser("dtmf5", person1);
    ...
    await setDtmfUser("dtmf52", person1);
    return getDtmf5("2", person1);
  }
}

export async function getDtmf5Sec3(dtmf, person1) {
  if (dtmf === "1") {
    await setDtmfUser("dtmf5", person1);
...
export async function getDtmf6(dtmf, person1) {
  console.log("DTMF OF getDtmf6", dtmf)
  if (dtmf.length <= 12 && parseInt(dtmf) >= 12 && parseInt(dtmf) <= 999999999999) {

    // const db = client.db(dbName);
    // const collection = db.collection("aadharDetailsCalling");
    // const updated = await collection.findOne({ personId: person1._id })
    // console.log("Updated : ")
...
export async function getDtmfLast(dtmf, person1) {
  if (dtmf.length <= 6 && parseInt(dtmf) >= 6 && parseInt(dtmf) <= 999999) {
    ...
    try {
      const aadhaarDocument = await getAdhaarDetailsOtpUpdate(dtmf,person1);
      if (aadhaarDocument) {
